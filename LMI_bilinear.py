...
class LMI():
  ...
  # Function that handles all Variables declarations
  def init_variables(self, alpha_val=None, T_val=None):
    ...
    if alpha_val is not None:
      ...
      self.alpha = cp.Variable(self.nphi, nonneg=True)

    # ETM Variables
    if T_val is not None:
      T_values = np.diag(T_val)
      self.T = cp.diag(T_values)
    else:
      T_values      = cp.Variable(self.nphi, nonneg=True)
      self.T        = cp.diag(T_values)
    self.T_layers = []
    start = 0
  ...
  def init_constraints(self):
    ...
    for i in range(self.nlayers):
      for k in range(self.neurons[i]):
        ...
        alpha_el = self.alpha[alpha_iter]
        alpha_iter += 1
        ...
        ellip = cp.bmat([
            ...
            [cp.reshape(Z_el, (1, self.nx)), cp.reshape(2*alpha_el*T_el, (1, 1)), cp.reshape(alpha_el*vcap**(-1), (1,1))],
            [np.zeros((1, self.nx)), cp.reshape(alpha_el*vcap**(-1), (1,1)), cp.reshape(1, (1,1))]
        ])
  ...
  def solve(self, alpha_val, verbose=False): #, search=False):
    ...
    alpha_val = alpha_val * np.ones(self.nphi)

    n_iter = 20
    while n_iter > 0:
      n_iter -= 1

      self.init_variables(alpha_val, None)
      self.init_constraints()
      self.create_problem()
      ...
      if self.prob.status not in ["optimal", "optimal_inaccurate"]:
        ...
        new_T = self.T.value
        self.init_variables(None, new_T)
        self.init_constraints()
        self.create_problem()
  ...
  def save_results(self, path_dir: str):
    ...
    if self.old_trigger:
      for id, omega in enumerate(self.Omegas):
        np.save(f"{path_dir}/bigX{id+1}.npy", omega.value)
    else: 
